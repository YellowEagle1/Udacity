...
it('can add the star name and star symbol properly', async() => {
    ...
    let starId = 6;
	await instance.createStar('awesome star', starId, {from: user1});
	assert.notEqual(await instance.name.call(), 'awesome star')
});
...
it('lets 2 users exchange stars', async() => {
	...
	let check = await instance.ownerOf.call(starId1) != user1 && await instance.ownerOf.call(starId2) != user2;
	assert.isTrue(check);
});
...
it('lets a user transfer a star', async() => {
	...
	let check = await instance.ownerOf.call(starId1) != user1;
	assert.isTrue(check);	
});
